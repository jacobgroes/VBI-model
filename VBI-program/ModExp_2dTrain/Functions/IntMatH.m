function [b] = IntMatH(pos,X,X3,T,e,bg,intw,l,la,S,nt,ll)
%**************************************************************************
% File: IntMatH.m
%   Creates the horizontal interaction matrix 
% Syntax:
%   b = IntMatH(pos,X,X3,T,e,bg,intw,l,la,S,nt)
% Input:
%   pos : Wheel positions
...
vno = ceil(intw(i)/2);
% Present wheel
wno = ((1+intw(i)/2)-vno)*2;

% Weighting of equivalent wheel mass
if wno == 1
    w = ll(2,1,vno)/(sum(ll(:,1,vno)));
elseif wno == 2
    w = ll(1,1,vno)/(sum(ll(:,1,vno)));
end
