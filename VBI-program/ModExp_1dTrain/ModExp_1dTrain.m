...
%   1d-mass-spring-damper vehicles are constructed in the input section.
% Input:
...
% Initial distances from node 1 of the bridge model to the wheels of each
% vehicle. The number of elements in the initial vector corresponds to the
% number of vehicles in the analysis.
initial = [0]';
...
vtpar = [30:5:500]; % Velocities for parametric study - ANALYSIS 2
...
% Train masses mt = [mt_1 mt_2 .. mt_np]'
mt = ones(np,1)*2e4;

% Wheel masses mw = [mw_1 mw_2 .. mw_np]'
mw = ones(np,1)*100;

% Train suspension stiffness kt = [kt_1 kt_2 .. kt_np]'
kt = ones(np,1)*4e6;

% Train suspension damping coefficient ct = [ct_1 ct_2 .. ct_np]'
ct = ones(np,1)*15e3;
...
% Train system matrices (diagonal matrices np x np) 
[Mt,mw,Kt,Ct] = assemTrain(mt,mw,kt,ct,np);

% -------------------------------------------------------------------------
% NODAL BRIDGE LOADS

% Stationary dynamic loads
Fi = zeros(nno*6,N+1);
%Fi((nno+1)/2*6-4,:) = -sin(2*pi/2*tt)*2e4;
...
% system matrices M, K, and C, along with the train-bridge load vector in 
% each time step.
...
[xb,vb,ab,xxv,xxh,Fw,xtr,vtr,atr] = MainProgram(n,nno,X,X3,T,gamma,...
    beta,dt,N,tt,initial,np,L,vt,e,eh,Mt,mw,Kt,Ct,mm,km,cm,g,Fi,S);
...
for j=1:length(vtpar)
    [xb(:,:,j),vb(:,:,j),ab(:,:,j),~,~,Fw(:,:,j),xtr(:,:,j),...
      vtr(:,:,j),atr(:,:,j)] = MainProgram(n,nno,X,X3,T,gamma,...
      beta,dt,N,tt,initial,np,L,vtpar(j),e,eh,Mt,mw,Kt,Ct,mm,km,cm,g,Fi,S);
...
% Fw: Contact forces for each train velocity (np x (N+1) x length(vtpar))

% xtr,vtr,atr: Vehicle response for each train velocity (1 x (N+1) x 
%              length(vtpar))
...
[z] = savedata(np,xb,vb,ab,xtr,vtr,atr,Fw,tt);
...
plotfigures(tt,xb,vb,ab,X,Fw,xxv,xxh,xtr,atr,z,np,plotfig)
...
animation = 1; % animation == 1 => video, animation ~= 1 => no animation

% Scaling for each animation plot. Note that the horizontal translation
% animation is only illustrated for displacement above 1e(-9).
...
scaley = 1; % Scale transverse displacement
...
posf = position(N,np,initial,vt,tt,X,T);

% Animation
animation1d(scalev,rr,scales,scaleh,scaley,dispw,rw,u,X,T,animation,z,...
    xb,nno,N,np,posf,xxv,xxh,e,dt)
